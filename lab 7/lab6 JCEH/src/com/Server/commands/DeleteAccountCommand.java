...
public class DeleteAccountCommand extends AbstractCommand {
    ...
    private final ServerConn currentConnection;

    public DeleteAccountCommand(ServerConn currentConnection) {
    ...
    public synchronized String execute(String[] args) {
        try {
            ...
            email.validate();
            try (java.sql.Connection connection = KingDatabaseMan.getInstance().getConnection()) {
                ...
                try {
                    request.setString(2, KingPasswordMan.getHash(args[1], "SHA1"));
                ...
                if (request.executeUpdate() == 1) {
                    try {
                        ...
                        //COllectionManager.CollectionManager manager = COllectionManager.CollectionManager.getInstance();
                        CollectionManager.getInstance().getFlats().removeIf(p -> p.getId()
